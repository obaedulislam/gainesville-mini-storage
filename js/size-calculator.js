$(document).ready(function () {
    ...
    $(".increase").click(function () {
        ...
        $(".count").each(function () {
            var qty = $(this).val();
            if (qty > 0) {
                var size = $(this).parent().next().children(".increase").attr("data-size");
        ...
        var imgFIleUrl = "images/size-guide/storage-calculator/" + imgFileName + ".webp";
    ...
    $(".decrease").click(function () {
        ...
        if (sizeValue > 0) {
            ...
            if (info.percentage_full > 0) {
                ...
                $("#containerImg").css("background-image", "unset");
            }
        }
    });
})
...
function sizeSuggestion(furnitureTQF) {
    ...
    for (let i = 0; i < myArray.length; i++) {
        if (furnitureTQF < myArray[i].net_usable_vol) {
            var percentFull = getPercentageFull(furnitureTQF, myArray[i].net_usable_vol);
            percentFull = roundNumber(percentFull);
            x = { size_suggestion: myArray[i].description, percentage_full: percentFull };
            break;
        }
    }
    return x;
